...
#ifdef CYGDBG_HG_DUMP_ALL_THREADS_INFO
// HG wujy add functions begin
// Return the current saved state for this thread.
inline HAL_SavedRegisters *Cyg_HardwareThread::hg_get_saved_context()
{
    HAL_SavedRegisters *regs;
    if( hg_saved_context != 0 ) regs = hg_saved_context;
    else HAL_THREAD_GET_SAVED_REGISTERS( stack_ptr, regs );
    return regs;
}

inline void Cyg_HardwareThread::hg_set_saved_context(HAL_SavedRegisters *ctx)
{
   hg_saved_context = ctx;
}
#endif
// HG wujy add functions end
