...
#if defined(CYGHWR_HAL_ARM_FPU_D32)
#define		FPU_D_REG_NUM	(32)
#else
#define		FPU_D_REG_NUM	(16)
#endif
...
typedef struct 
{
    ...
    cyg_uint64  f[FPU_D_REG_NUM];                 // 32x64 NEON or VFP registers
...
{                                                                               \
    for( _i_ = 0; _i_ < FPU_D_REG_NUM; _i_++ ) (_regs_)->f[_i_] = (_id_)|0xFF00|_i_;       \
...
#if defined(CYGHWR_HAL_ARM_NEON) || defined(CYGHWR_HAL_ARM_FPU)
#define CYGNUM_HAL_STACK_FRAME_SIZE ((4 * 20) + (2 * 20) + (8 * FPU_D_REG_NUM))
#else
