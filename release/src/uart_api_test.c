...
hang();
    ...
    for (i = 0;  i < 100;  i++) {
        len = (i % 10) + 1;
        UartWrite(1, filler, &len);
        len = strlen(long_msg);
        res = UartWrite(1, long_msg, &len);
        if (res != ENOERR) {
    ...
    len = strlen(prompt);
    UartWrite(1, prompt, &len);
#ifdef CYGFUN_KERNEL_API_C
    stamp = timeout(1000, do_abort, handle);
#endif
    res = UartRead(1, in_msg, &in_msglen);
    if (res != ENOERR) {
