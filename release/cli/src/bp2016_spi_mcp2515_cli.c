//==========================================================================
//
//        bp2016_spi_mcp2515_cli.c
//
//        spi mcp2515 cli test for BP2016
//
//==========================================================================
// Author(s):
// Contributors:  BSP
// Date:          2019-05-05
// Description:   spi cli mcp2515 device for bp2016
//===========================================================================
//                                INCLUDES
//===========================================================================
...
static spi_dev_config_t mcp2515_8bit_config = {
    .spi_dw_cfg.index = 1,
    .spi_dw_cfg.cs = 0,
    .spi_dw_cfg.cpol = 1,
    .spi_dw_cfg.cpha = 1,
...
#define     MCP2515_IRQ_GPIO_NUM        12
#define     MCP2515_RESET_GPIO_NUM      11
static cyg_uint8 wrbuf[0x80] = {0};
static cyg_uint8 rdbuf[0x80] = {0};
static void *handler = NULL;
static unsigned int mcp_rst_gpio = MCP2515_RESET_GPIO_NUM;

#define MCP_CMD_READ    0x03
#define MCP_CMD_WRITE   0x02

int do_mcp2515_testing(shell_cmd_t *cptr, const unsigned int argc, const char **argv)
{
    int res = 0;
    cyg_uint32 txlen = 0;
    cyg_uint32 rxlen = 1;
    ...
    if (strcmp(argv[1], "reset") == 0) {
        gpio_api_init();
        gpio_api_set_output(mcp_rst_gpio);
        gpio_api_set_low(mcp_rst_gpio);
        cyg_thread_delay(2);
        gpio_api_set_high(mcp_rst_gpio);
    }

    if (strcmp(argv[1], "init") == 0) {
        hal_iomux_spi1_en(1, 0);
        handler = SpiInit(&mcp2515_8bit_config);
        if (handler == NULL) {
