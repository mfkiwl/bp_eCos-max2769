...
#define HG_FLASH_BLOCK_ERASE   0
#define HG_FLASH_SECTOR_ERASE  1
...
struct cyg_flash_dev_funs {
  ...
  int     (*flash_erase_block) (struct cyg_flash_dev *dev, 
                                cyg_flashaddr_t block_base, int erase_type);
  ...
  int     (*flash_block_unlock) (struct cyg_flash_dev *dev, 
                                 const cyg_flashaddr_t block_base);
  int     (*flash_region_lock) (struct cyg_flash_dev *dev, cyg_uint32 region_num);
  int     (*flash_region_unlock) (struct cyg_flash_dev *dev, cyg_uint32 region_num);
  int     (*flash_global_lock) (struct cyg_flash_dev *dev);
  int     (*flash_global_unlock) (struct cyg_flash_dev *dev);
#endif    
  int     (*flash_read_id) (struct cyg_flash_dev *dev);
  int     (*flash_chip_erase) (struct cyg_flash_dev *dev);
  unsigned int  (*flash_get_sector_size) (struct cyg_flash_dev *dev);
};
...
// wwzz add _read_id_ and _chip_erase_  20181024
#ifdef CYGHWR_IO_FLASH_BLOCK_LOCKING
# define CYG_FLASH_FUNS(_funs_, _init_, _query_ , _erase_, _prog_ , _read_, _lock_, _unlock_, region_lock, region_unlock, global_lock, global_unlock, _read_id_, _chip_erase_, _get_sector_) \
...
{										\
	...
	.flash_region_lock       = region_lock,   \
	.flash_region_unlock     = region_unlock,	\
	.flash_global_lock       = global_lock,   \
	.flash_global_unlock     = global_unlock,	\
	.flash_read_id           = _read_id_,	\
	.flash_chip_erase        = _chip_erase_,	\
	.flash_get_sector_size   = _get_sector_	\
}
#else
# define CYG_FLASH_FUNS(_funs_, _init_, _query_ , _erase_, _prog_ , _read_, _lock_, _unlock_, _get_sector_) \
struct cyg_flash_dev_funs _funs_ =      \
{										\
	.flash_init             = _init_,   \
	.flash_query            = _query_,  \
