...
static size_t 
flash_sector_size(struct cyg_flash_dev *dev, const cyg_flashaddr_t addr)
{
  CYG_ASSERT((addr >= dev->start) && (addr <= dev->end), "Not inside device");
  return dev->funs->flash_get_sector_size(dev);
}

// Return the size of the block which is at the given address
__externC size_t
cyg_flash_sector_size(const cyg_flashaddr_t flash_base) 
{
  struct cyg_flash_dev *    dev;
  int                       stat;
  ...
  dev = find_dev(flash_base, &stat);
  if (!dev) return stat;
  return flash_sector_size(dev, flash_base);
}

// Return the first address of a sector. The flash might not be aligned
// in terms of its sector size. So we have to be careful and use
// offsets.
static inline cyg_flashaddr_t 
flash_sector_begin(cyg_flashaddr_t addr, struct cyg_flash_dev *dev)
{
  size_t sector_size;
  cyg_flashaddr_t offset;
  
  sector_size = flash_sector_size(dev, addr);
...
__externC int 
cyg_flash_sector_erase(cyg_flashaddr_t flash_base, 
                size_t len, 
                cyg_flashaddr_t *err_address)
{
  cyg_flashaddr_t sector, end_addr;
  struct cyg_flash_dev * dev;
  size_t erase_count;
  int stat = CYG_FLASH_ERR_OK;
  ...
  // erase can only happen on a sector boundary, so adjust for this
  sector         = flash_sector_begin(flash_base, dev);
  erase_count   = (end_addr + 1) - sector;

  CHATTER(dev, "... Erase from %p-%p: ", (void*)sector, (void*)end_addr);
  
  //diag_printf("%s %d  addr 0x%x len %d \n", __func__, __LINE__, flash_base, len);
  ...
  while (erase_count > 0) {
    ...
    unsigned char *dp;
    bool erased = false;
    size_t sector_size = flash_sector_size(dev, sector);

    if(!sector_size)
    {
        printf("this chip not support sector erase!\n");
        return false;
    }

    // Pad to the sector boundary, if necessary
    if (erase_count < sector_size) {
        erase_count = sector_size;
    }
    ...
    if (!erased) {
		//diag_printf("%s %d  erase sector base 0x%x\n", __func__, __LINE__, sector);
      stat = dev->funs->flash_erase_block(dev,sector, HG_FLASH_SECTOR_ERASE);
    }
    if (CYG_FLASH_ERR_OK != stat) {
        if (err_address)
            *err_address = sector;
  ...
  if (len > (dev->end + 1 - flash_base)) {
    return cyg_flash_sector_erase(dev->end+1, 
                ...
                cyg_flashaddr_t *err_address)
{
  ...
  while (erase_count > 0) {
    ...
    if (!erased) {
		//diag_printf("%s %d  erase block base 0x%x\n", __func__, __LINE__, block);
      stat = dev->funs->flash_erase_block(dev,block, HG_FLASH_BLOCK_ERASE);
    }
    if (CYG_FLASH_ERR_OK != stat) {
        if (err_address)
            *err_address = block;
...
// wwzz add chip_erase 20181024
__externC int 
cyg_flash_chip_erase(cyg_flashaddr_t flash_base)
{
  struct cyg_flash_dev * dev;
  int stat = CYG_FLASH_ERR_OK;
  HAL_FLASH_CACHES_STATE(d_cache, i_cache);
  ...
  stat = dev->funs->flash_chip_erase(dev);
  if (CYG_FLASH_ERR_OK != stat) {
      //if (err_address)
...
// Return the flashid
__externC size_t
cyg_flash_read_id(const cyg_flashaddr_t flash_base) 
{
  struct cyg_flash_dev *    dev;
  int                       stat;
  ...
  if (!dev) return stat;
  LOCK(dev);
  stat = dev->funs->flash_read_id(dev);
  UNLOCK(dev);
  return stat;
}
                  ...
                  cyg_flashaddr_t *err_address)
{
  ...
  while (write_count > 0) {
	...
	//diag_printf("%s %d  write addr 0x%x len %d ram base 0x%x\n", __func__, __LINE__, addr, this_write, ram);
               ...
               cyg_flashaddr_t *err_address)
{
  ...
  if (! dev->funs->flash_read) {
      ...
      while (read_count > 0) {
		  ...
		  //diag_printf("%s %d  read addr 0x%x len %d ram base 0x%x\n", __func__, __LINE__, addr, this_read, ram);
...
__externC int 
cyg_flash_region_lock(cyg_flashaddr_t flash_base, cyg_uint32 region)
{
	int err;
	struct cyg_flash_dev * dev;
	int stat = CYG_FLASH_ERR_OK;
	...
	if (!dev->funs->flash_region_lock) return CYG_FLASH_ERR_INVALID;

	err = dev->funs->flash_region_lock(dev, region);
...
__externC int 
cyg_flash_region_unlock(cyg_flashaddr_t flash_base, cyg_uint32 region)
{
	int err;
	struct cyg_flash_dev * dev;
	int stat = CYG_FLASH_ERR_OK;
	...
	if (!dev->funs->flash_region_unlock) return CYG_FLASH_ERR_INVALID;

	err = dev->funs->flash_region_unlock(dev, region);
...
__externC int 
cyg_flash_global_lock(cyg_flashaddr_t flash_base)
{
	int err;
	struct cyg_flash_dev * dev;
	int stat = CYG_FLASH_ERR_OK;
	...
	if (!dev->funs->flash_global_lock) return CYG_FLASH_ERR_INVALID;

	err = dev->funs->flash_global_lock(dev);
...
__externC int 
cyg_flash_global_unlock(cyg_flashaddr_t flash_base)
{
	int err;
	struct cyg_flash_dev * dev;
	int stat = CYG_FLASH_ERR_OK;
	...
	if (!dev->funs->flash_global_unlock) return CYG_FLASH_ERR_INVALID;

	err = dev->funs->flash_global_unlock(dev);
...
// check if we have any overlap of the addresses.
__externC int 
cyg_flash_scan(cyg_flash_printf *pf)
{
  int err;
  struct cyg_flash_dev * dev;
  char buf[32]={0,};
  ...
  for (dev = &cyg_flashdevtab[0]; dev != &cyg_flashdevtab_end; dev++) {
    ...
    err = dev->funs->flash_query(dev, buf, 32);
    if (err != CYG_FLASH_ERR_OK) {
      continue;
    }
    CYG_ASSERT(dev->funs, "No flash functions");
