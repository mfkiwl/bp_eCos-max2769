                       ...
                       cyg_uint32* len)
{
  ...
  switch (key) {
// wwzz add 20181024
  case CYG_IO_GET_CONFIG_FLASH_CHIP_ERASE:
    {
        if (*len != sizeof( cyg_io_flash_getconfig_erase_t ) )
            return -EINVAL;
        cyg_io_flash_getconfig_erase_t *e = (cyg_io_flash_getconfig_erase_t *)buf;
        e->flasherr = cyg_flash_chip_erase(dev->start);
        return ENOERR;
    }

  case CYG_IO_GET_CONFIG_FLASH_READ_ID:
    {
        *((cyg_uint32 *)buf) = cyg_flash_read_id(dev->start);
        return ENOERR;
    }
  ...
  case CYG_IO_GET_CONFIG_FLASH_ERASE:
    {
...
#endif
      {
		...
		//diag_printf("%s, 0x%x -- len 0x%x\n", __func__, startpos, e->len);
        // Unlike some other cases we _do_ do bounds checking on this all the time, because
  ...
  case CYG_IO_GET_CONFIG_FLASH_SECTOR_ERASE:
    {
#ifdef CYGPKG_INFRA_DEBUG // don't bother checking this all the time
      if (*len != sizeof( cyg_io_flash_getconfig_erase_t ) )
        return -EINVAL;
#endif
      {
        ...
        e->flasherr = cyg_flash_sector_erase( startpos, e->len, &e->err_address );
      }
      return ENOERR;
    }
  ...
  case CYG_IO_GET_CONFIG_FLASH_SECTOR_SIZE:
    {
      cyg_io_flash_getconfig_blocksize_t *b =
        (cyg_io_flash_getconfig_blocksize_t *)buf;
      cyg_flashaddr_t pos = dev->start + b->offset;
      ...
      if ( *len != sizeof( cyg_io_flash_getconfig_blocksize_t ) )
        return -EINVAL;
#endif  
      b->block_size = cyg_flash_sector_size(pos );
...
__externC int
flashiodev_region_lock(cyg_io_handle_t handle, cyg_uint32 region)
{
  struct cyg_devtab_entry *tab = (struct cyg_devtab_entry *)handle;
  struct flashiodev_priv_t *dev = (struct flashiodev_priv_t *)tab->priv;
  ...
  Cyg_ErrNo err = ENOERR;
  int flasherr;
  cyg_flashaddr_t startpos = dev->start;
  
  CYG_ASSERT( dev->handle.handlers == &cyg_io_flashdev_ops, "bad flash operation link" );
  CYG_ASSERT( ((struct flashiodev_priv_t*)(dev->handle.priv))->valid, "operation on not valid flash device instance" );

  flasherr = cyg_flash_region_lock( startpos, region);
...
__externC int
flashiodev_region_unlock(cyg_io_handle_t handle, cyg_uint32 region)
{
  struct cyg_devtab_entry *tab = (struct cyg_devtab_entry *)handle;
  struct flashiodev_priv_t *dev = (struct flashiodev_priv_t *)tab->priv;
  ...
  flasherr = cyg_flash_region_unlock( startpos, region);
...
__externC int
flashiodev_global_lock(cyg_io_handle_t handle)
{
  struct cyg_devtab_entry *tab = (struct cyg_devtab_entry *)handle;
  struct flashiodev_priv_t *dev = (struct flashiodev_priv_t *)tab->priv;
  ...
  flasherr = cyg_flash_global_lock(dev->start);
...
__externC int
flashiodev_global_unlock(cyg_io_handle_t handle)
{
  struct cyg_devtab_entry *tab = (struct cyg_devtab_entry *)handle;
  struct flashiodev_priv_t *dev = (struct flashiodev_priv_t *)tab->priv;
  ...
  flasherr = cyg_flash_global_unlock(dev->start);
