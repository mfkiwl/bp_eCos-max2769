...
#include <cyg/hal/hal_diag.h>  // wwzz add for arch_timer counter
#endif
...
static cyg_ucount32   thread_switches_table[CYGNUM_KERNEL_TREAHD_STATS][CYGNUM_KERNEL_TREAHD_STATS];
externC void * cyg_get_thread_trans_table(void) 
{
	return  &thread_switches_table[0][0];
}
#endif
...
#ifdef CYGFUN_KERNEL_THREADS_SWITCH_STATS
				if((current->get_unique_id() - 1 < CYGNUM_KERNEL_TREAHD_STATS) && (next->get_unique_id() -1 < CYGNUM_KERNEL_TREAHD_STATS))
					thread_switches_table[current->get_unique_id()-1][next->get_unique_id() - 1] ++;
#endif
...
{
    CYG_REPORT_FUNCTION();
    // wwzz add static flag for record start of thread
#ifdef CYGFUN_KERNEL_THREADS_CPULOAD
    static bool flag_thread_start = false;
#endif
    ...
    // wwzz add start tick of thread here
#ifdef CYGFUN_KERNEL_THREADS_CPULOAD
    if(false == flag_thread_start){
        flag_thread_start = true;
        tick_sch_start = HAL_GET_COUNTER;
    }
    next->tick_start = HAL_GET_COUNTER;
#endif
