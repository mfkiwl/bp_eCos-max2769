...
externC void hal_interrupt_dsr_enter(cyg_int32 vector);
externC void hal_interrupt_irq_leave(cyg_int32 vector);
#endif 
void
...
#ifdef  CYGINT_HAL_INTERRUPT_TIME_STAT 
        hal_interrupt_dsr_enter(intr->vector);
#endif
        intr->dsr(intr->vector, count, (CYG_ADDRWORD)intr->data);
#ifdef  CYGINT_HAL_INTERRUPT_TIME_STAT 
        hal_interrupt_irq_leave(intr->vector);
#endif
        if (!next)
            break;
...
//    CYG_REPORT_FUNCTION();
    ...
    Cyg_Scheduler::get_current_thread()->hg_set_saved_context(regs);
#endif
...
#ifdef CYGDBG_HG_DUMP_ALL_THREADS_INFO
    Cyg_Scheduler::get_current_thread()->hg_set_saved_context(0);
#endif
