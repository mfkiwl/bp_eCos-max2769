...
Cyg_ErrNo UartWrite(cyg_uint32 id, void *wbuf, cyg_uint32 *wlen)
{
    ...
    if(uart_api_manager_st[id].handle == NULL)
    {
        uart_printf("%s para error, no dev found!\n", __FUNCTION__);
        return -EINVAL;
    }
    ...
    // normal write, timeout set to 0x7FFFFFFF, wait forever
    cyg_io_set_config(uart_api_manager_st[id].handle, CYG_IO_SET_CONFIG_SERIAL_WRITE_TIMEOUT, (void *)timeout, &len);
...
Cyg_ErrNo UartRead(cyg_uint32 id, void *rbuf, cyg_uint32 *rlen)
{
    ...
    // normal read, timeout set to 0x7FFFFFFF, wait forever
    cyg_io_set_config(uart_api_manager_st[id].handle, CYG_IO_SET_CONFIG_SERIAL_READ_TIMEOUT, (void *)timeout, &len);
...
Cyg_ErrNo UartReadTimeOut(cyg_uint32 id, void *rbuf, cyg_uint32 *rlen, cyg_uint32 timeout)
{
    Cyg_ErrNo res = 0;
    cyg_uint32 len = sizeof(cyg_uint32);
...
Cyg_ErrNo UartGetBufInfo(cyg_uint32 id, cyg_uart_buf_info_t *info)
{
    cyg_uint32 len = sizeof(cyg_uart_buf_info_t);

	if((NULL == info) || (id >= HWP_SERIAL_PORT_NUM)){
        uart_printf("%s para error \n", __FUNCTION__);
        return -EINVAL;
	}
    if(CYGNUM_UART_INIT_TYPE_SYS != uart_api_manager_st[id].init_type){
    ...
    uart_printf("%s: config_addr = 0x%x\n", __FUNCTION__, (cyg_uint32)info);

    return cyg_io_get_config(uart_api_manager_st[id].handle, CYG_IO_GET_CONFIG_SERIAL_BUFFER_INFO,
                           (void *)info, &len);
}
...
cyg_int32 UartPollingInit(cyg_uint32 id, cyg_uint32 baud)
{
    ...
    cyg_int32  res;
    uart_printf("%s id(%d) , baud(%d)\n", __FUNCTION__, id, baud);
	if(id >= HWP_SERIAL_PORT_NUM){
        uart_printf("%s id(%d) error \n", __FUNCTION__, id);
        return -EINVAL;
	}
